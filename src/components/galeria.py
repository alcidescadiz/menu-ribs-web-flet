...
def agregar_al_carrito(e, page, producto, cambiar_pagina):
    ...
    mostrar_notificacion(
        page, f"Agregado {str(producto["title"])} a su pedido", cambiar_pagina
    )
    page.update()
...
def galeria(page, titulo, lista_productos, cambiar_pagina):
    ...
    def boton_agregar_al_carrito(lista_producto):
        return ft.ElevatedButton(
            ...
            on_click=lambda e, item=lista_producto,: agregar_al_carrito(
                e, page, item, cambiar_pagina
            ),
    ...
    if titulo == "Burger":
        boton_abrir_modal = ft.ElevatedButton(
            "+1 Extras", on_click=lambda e: page.open(modal), bgcolor=ft.Colors.WHITE
        )
            ...
            ft.Container(
                content=ft.Column(
                    [
                        ...
                        ft.Text(
                            lista_producto["description"],
                            size=16,
                            color=ft.Colors.GREY_400,
                            text_align=ft.TextAlign.JUSTIFY,
                        ),
                        ft.Text(
                            lista_producto["price"],
                            size=16,
                            color=ft.Colors.GREY_400,
                            text_align=ft.TextAlign.CENTER,
                        ),
                        ft.Column(
                            [
                                ft.Row(
                                    controls=[
                                        ft.Container()  if lista_producto.get("delivery") =="False"  else boton_agregar_al_carrito(lista_producto),
